...
void Main()
{
	...
	while (System::Update())
	{
		// key input
		{
			...
			if (KeyQ.down()) { lastSize /= 2; updateMandelbrotTex(); }
			if (KeyA.down()) { lastSize *= 2; updateMandelbrotTex(); }

			if (KeyLeft .down()) { lastOffset.x -= moveValue * lastSize / defaultSize; updateMandelbrotTex(); }
			if (KeyRight.down()) { lastOffset.x += moveValue * lastSize / defaultSize; updateMandelbrotTex(); }
			if (KeyUp   .down()) { lastOffset.y -= moveValue * lastSize / defaultSize; updateMandelbrotTex(); }
			if (KeyDown .down()) { lastOffset.y += moveValue * lastSize / defaultSize; updateMandelbrotTex(); }

			if (KeyZ.down()) { if (++currentColor == colorTable.end()) currentColor = colorTable.begin(); updateMandelbrotTex(); }

			if (KeySpace.down()) { updateMandelbrotTex(); }
		}

		// draw mandelbrot set
		{
			const ScopedRenderStates2D rstat(SamplerState::ClampLinear);
		...
		// draw UI
		{
			...
			pos.y = UIFont(U"[W, S]: move value ({0:f})"_fmt(moveValue)             ).draw(pos).bl().y;
			pos.y = UIFont(U"[3, E, D, C]: num operation ({})"_fmt(opCount)         ).draw(pos).bl().y;
